...
//(considering the fact if there is not enough d_energy as a result of low number of particle or low floctuation 
...
__global__ void MBS (double* scalefactor , int* n , double* e, curandState *state,int Nc )
{
    ...
    if (tid<Nc)
    {
        ...
        if (e[tid] > 1e-2)
        {
            ...
            double ek;
            gamrnd_d(&ek, &params, &localState);//it generates a random variable ek using a gamma distribution with parameters f and 1
            scalefactor[tid] = sqrt(ek/ e[tid]);//Assigns scalefactor[tid] to the square root of the ratio of ek to e[tid]
        }
        else 
        {
            scalefactor[tid] = 1;
